...
However, Solidity provides a built-in mechanism to obtain this value directly through the language syntax itself, resulting in a precomputed and therefore more gas-efficient representation (see Figure \ref{code:erc165}).
...
\begin{enumerate}
\item \texttt{forge}, which enables compilation, testing, debugging, deployment, and verification of smart contracts.
\item \texttt{anvil}, which allows the instantiation of a local development node by forking the selected blockchain.
\item \texttt{cast}, a versatile command-line tool for interacting with on-chain applications.
\end{enumerate}

\texttt{forge} is the primary tool used when developing in Solidity.

forge initializes a new project (structured as a directory), where smart contracts are placed in the \texttt{src} folder.
...
\begin{enumerate}
\item Programmatically, by binding Solidity methods in Rust (or in JavaScript in other development environments).
\item By verifying contract ownership on external platforms (such as Etherscan) and interacting directly from a web browser via a connected wallet.
\end{enumerate}
